...
export default class JobOpeningsList extends React.Component {
  ...
  componentDidMount() {
    fetch("http://hellotechnigo.comprendwebsites.net/api/jobs")
      .then(response => {
  ...
  fetchDepartments = () => {
    fetch("http://hellotechnigo.comprendwebsites.net/api/departments")
      .then(response => {
        return response.json()
      })
      .then(json => {
  ...
  uniqueCities = () =>
    Array.from(
      new Set(
        this.state.allJobListings.map(job => {
          return job.city
        })
      )
    )
