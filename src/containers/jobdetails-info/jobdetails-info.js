...
export default class JobDetailsInfo extends React.Component {
  ...
  componentDidMount() {
    ...
    fetch("http://hellotechnigo.comprendwebsites.net/api/users")
      .then(response => {
        return response.json()
      })
      .then(json => {
  ...
  render() {
    if (
      Object.keys(this.state.allJobListings).length > 0 &&
      Object.keys(this.state.allColleagues).length > 0
    ) {
      let quote
      ...
      return (
        <div>
          ...
          <div className="top-image-box">
            <img
              src="https://www.comprend.com/files/pages/169/594257/dt/Home_02-ws-1920x800.jpg"
              alt="comprend interior"
            />
          </div>
          ...
          <div className="two-column">
            ...
            <p className="column-text">
              <h3>Who are you?</h3>
              {this.state.allJobListings.aboutYou}
            </p>
          </div>
          <br />
          ...
          <div className="colleague-group">
            <Link to="/colleagues">
              <Colleague
                source={this.state.allColleagues[0].pictureUrl}
                name={this.state.allColleagues[0].name}
              />
            </Link>
            <Link to="/colleagues">
              <Colleague
                source={this.state.allColleagues[1].pictureUrl}
                name={this.state.allColleagues[1].name}
              />
            </Link>
            <Link to="/colleagues">
              <Colleague
                source={this.state.allColleagues[2].pictureUrl}
      ...
      return <div>Please wait while we find available jobs.</div>
    }
  }
}
